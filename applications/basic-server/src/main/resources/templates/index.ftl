...
<@layout.header>
    ...
    <div class="w-full flex items-center justify-center">
        <table class="table-auto border my-3">
          <thead>
            <tr>
              <th class="font-bold p-2 border-b text-left">Team Members</th>
            </tr>
          </thead>
          <tbody>
            <#list users?reverse as user>
                <tr>
                  <td class="p-2 border-b text-left"><a href="/form/${user.id}"  class="font-medium text-blue-600 dark:text-blue-500 hover:underline">${user.title}</a></td>
                </tr>
            </#list>
          </tbody>
        </table>
